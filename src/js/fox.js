...
export class Fox {
  ...
  _listenMouse() {
    ...
    document.addEventListener('mouseenter', () => {
      this.isSleeping = false;
      this.ears.forEach(ear => ear.position.y = 0.55);
      setTimeout(() => this.ears.forEach(ear => ear.position.y = 0.45), 200);
    });
  ...
  _doBoop() {
    this.isBooped = true;
    this.currentRotation.x -= 0.4;
    this.nose.scale.set(1.6, 1.6, 1.6);
    this.nose.material = this.mats.blush; 
    this.ears[0].rotation.z = -0.5; this.ears[1].rotation.z = 0.5;
    this.eyes.forEach(eye => eye.scale.y = 0.05);

    setTimeout(() => {
      this.isBooped = false;
      this.nose.scale.set(1, 1, 1);
  ...
  _doBlink() {
    if (this.isSleeping || this.isBooped) return; 
    this.eyes.forEach(eye => eye.scale.y = 0.1); 
    setTimeout(() => {
      if (!this.isSleeping && !this.isBooped) this.eyes.forEach(eye => eye.scale.y = 1);
    }, 150);
  }
  ...
  _startLoop() {
    const loop = () => {
      ...
      if (this.isSleeping) {
        this.targetRotation.x = 0.6; 
        this.targetRotation.y = 0;   
        this.eyes.forEach(eye => eye.scale.y = 0.1);
        this.foxGroup.position.y = Math.sin(time * 1.5) * 0.08 - 0.1; 
      } else {
        if (!this.isBooped) {
          // ── PERFECT TRACKING ──
          // The Y-axis (Pitch) must be inverted so pushing mouse up looks up.
          this.targetRotation.x = -this.raycastMouse.y * 0.4;
          this.targetRotation.y = this.raycastMouse.x * 0.6;
        }
      ...
      if (!this.isSleeping && !this.isBooped && Math.random() > 0.995) {
        this.ears[0].rotation.z = 0.25 + Math.random() * 0.3;
        setTimeout(() => { if (!this.isBooped) this.ears[0].rotation.z = 0.25; }, 100);
      }
